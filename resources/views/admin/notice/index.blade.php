                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            @if ($notice->is_active == 1)
                                <form action="{{ route('admin.notice.unpublish', $notice->id) }}" method="post"
                                    class="d-inline">
                                    @csrf
                                    <button type="submit" class="btn btn-warning btn-sm"><i class="fa fa-times"></i>
                                        Unpublish</button>
                                </form>
                            @else
                                <form action="{{ route('admin.notice.publish', $notice->id) }}" method="post"
                                    class="d-inline">
                                    @csrf
                                    <button type="submit" class="btn btn-success btn-sm"><i class="fa fa-times"></i>
                                        Publish</button>
                                </form>
                            @endif
                        </td>
