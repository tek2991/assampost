...
@forelse ($downloads as $k=>$download)
    <tr>
    ...
    <form action="{{route('admin.download.destroy',$download->id)}}" method="post" class="d-inline">
    @csrf
    @method('delete')
   ...
   <form action="{{route('admin.download.unpublish',$download->id)}}" method="post" class="d-inline">
    @csrf
    <button type="submit" class="btn btn-warning btn-sm"><i class="fa fa-times"></i> Unpublish</button>
    </form>
    @else
    <form action="{{route('admin.download.publish',$download->id)}}" method="post" class="d-inline">
    @csrf
